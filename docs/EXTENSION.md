...
Local builds may use a loopback `VITE_API_BASE_URL` (default `http://127.0.0.1:3000`).

Production zip creation must set a non-loopback API origin:

```sh
VITE_API_BASE_URL=https://api.example.com pnpm --filter @pricetruth/extension build
VITE_API_BASE_URL=https://api.example.com pnpm --filter @pricetruth/extension package
pnpm --filter @pricetruth/extension verify-package
```

`package` fails if `VITE_API_BASE_URL` is missing or points at localhost / 127.0.0.1 / ::1.
Do not invent the production URL; take it from the backend handoff after Devin deploys.

## Side panel states

`idle` | `unsupported` | `ambiguous` | `loading` (submitting/analyzing + generation) | `ready` | `error`

`ambiguous_price` extractionsions never ingest. Generation tokens discard stale API responses after navigation/variant changes.

Deal Score and Discount Integrity are always labeled separately. Insufficient confidence shows a learning card without numeric scores.
