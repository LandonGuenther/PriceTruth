...
| Error hygiene         | Global error handler returns `{ error: "internal_error", message: "Internal error" }` for 5xx; internals only logged; 4xx/429 logged at `warn` with code/status/message only                                                                                                                     | injected `Error("secret internal detail")` → 500 with sanitised body     |
| Internal endpoints    | `/internal/metrics` + `/internal/status` require `Authorization: Bearer <INTERNAL_API_TOKEN>`; sha256-hashed constant-time compare; 404 (not 401) when the token is unset or wrong so the routes aren't discoverable                                                                             | unset/missing/wrong token → 404 (ops.test.ts)                            |
| Rate limit            | `@fastify/rate-limit` per endpoint class, per IP: ingest 60/min (`POST /v1/observations`), read 240/min (`/v1/listings/*`), health 600/min (`/health`, `/readiness`, `/internal/*`); 429 body `{error:"rate_limited", message, retryAfterSeconds}`; keys on `request.ip` honouring `TRUST_PROXY` | 241st read / 61st ingest / 601st health → 429                            |
| CORS                  | `@fastify/cors`: no-`Origin` callers always allowed; `chrome-extension://<id>` allowed only for `ALLOWED_EXTENSION_IDS` when set (any extension id when unset — warned in staging/production); all other origins only if listed in `CORS_ORIGINS`. Never `*`                                     | allowed/disallowed ids, CORS_ORIGINS, preflight (contract.test.ts)       |
| Response headers      | `x-content-type-options: nosniff`, `referrer-policy: no-referrer` on all; `cache-control: no-store` on `/v1` + `/internal`; `x-request-id` echo only for `^[A-Za-z0-9._-]{1,128}$` (else regenerated)                                                                                            | header assertions + hostile request-id test                              |
| Protocol robustness   | duplicate query params → 400; 200-level nested JSON → 400; array body → 400; undersized `Content-Length` closes the socket without a crash; `requestTimeout` (`REQUEST_TIMEOUT_MS`, default 15 s) applied                                                                                        | protocol-level hardening group                                           |
| SSRF (enrichment)     | Best Buy fetch URL built only from `^\d{1,12}$` SKU against fixed `api.bestbuy.com` host; invalid SKU returns null before any fetch                                                                                                                                                              | `1/../../evil` etc. never reach fetch                                    |
...
3. **`pnpm audit --prod --audit-level=high`** (2026-09-11): 1 high —
   `deepmerge-ts <8.0.0` (prototype-pollution class, GHSA-ggr8-5vv4-36mx)
   reached only via the `prisma` CLI dev-tool chain
   (`@prisma/client > prisma > @prisma/config > deepmerge-ts`); it is not in the
   runtime server path. Accepted risk for now; revisit on the next Prisma
   upgrade.
...
- Dependency audit runs in CI (`pnpm audit --prod --audit-level=high`,
  continue-on-error until the finding above is resolved).
