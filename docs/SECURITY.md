...
| Rate limit            | `@fastify/rate-limit` per endpoint class, per IP: ingest 60/min (`POST /v1/observations`), read 240/min (`/v1/listings/*`), health 600/min (`/health`, `/readiness`); 429 body `{error:"rate_limited", message, retryAfterSeconds}`; keys on `request.ip` honouring `TRUST_PROXY` | 241st read / 61st ingest / 601st health → 429                            |
